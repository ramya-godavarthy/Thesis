...
%定义全局变量 
QuanJuValue(speech); 
present_speech=zeros(1,80);%当前处理语音 
new_speech=zeros(1,80);%后继语音，用于加窗 
total_speech=zeros(1,240);%加窗语音 
old_wsp=zeros(1,160);%(-143:0)过去的低通感知加权语音，用于开环基音时延搜索 
old_exc=zeros(1,160);%(-143:0)过去的激励，用于闭环基音时延搜索 
inmapa=[5, 1, 4, 7, 3, 0, 6, 2];%增益量化映射表 
inmapb=[4, 6, 0, 2,12,14, 8,10,15,11, 9,13, 7, 3, 1, 5];%增益量化映射表 
%/*----------------------------------------------------------------------* 
...
for i=1:L 
    ...
    new_speech=round(Pre_Process(new_speech));%预处理，针对新一帧 
    total_speech=[total_speech(81:end),new_speech];     
    present_speech=total_speech(121:200);%当前编码帧语音，80 speech samples 
